...
jQuery(document).ready(function ($) {
	...
	if ($('.wishlist_items_number').length > 0 && shopkeeper_catalogMode != 1) {

		/*
		**	Update wishlist counter on demand
		*/
		function shopkeeper_update_wishlist_count() {
			$('.wishlist_items_number').html(localStorage.shopkeeperWishlist);
		}
		
		/*
		**	Initialize counter; fetch via ajax if not exist, then update	
		*/
		if (!localStorage.shopkeeperWishlist)
		{
			$.ajax({
				url: shopkeeper_ajaxurl,
				type: "POST",
				data: {
					'action' : 'shopkeeper_update_wishlist_count'
				},
				success:function(data) {
					localStorage.shopkeeperWishlist = data;
					shopkeeper_update_wishlist_count();
				}
			});
		}
		else 
		...
		*/
		$('body').on( 'added_to_wishlist' , function(){
			localStorage.shopkeeperWishlist = Number(localStorage.shopkeeperWishlist) + 1;
			shopkeeper_update_wishlist_count()
		});
